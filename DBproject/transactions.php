...
try {
    ...
    if ($action == "insert_or_update" && $stock_id != "" && $user_id != "" && $quantity != 0 && $price != 0) {
        $insert_or_update = filter_input(INPUT_POST, 'insert_or_update');

        if ($insert_or_update == "insert") {
            insert_user($user_id, $stock_id, $quantity, $price, $timestamp);
        } else if ($insert_or_update == "update") {
            update_user($user_id, $stock_id, $quantity, $price, $timestamp);
        }
